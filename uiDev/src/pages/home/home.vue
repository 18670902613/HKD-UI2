...
<script>
    ...
    import { getUserData, getDepartmentsData, getNewsData, getServicesData, getCollectionsData } from '../../../data/getData.js'
    ...
    export default {
        ...
        created() {
            // 请求数据
            axios.all([this.getUserData(), this.getDepartmentsData(), this.getNewsData(), this.getServicesData(), this.getCollectionsData()])
                .then(axios.spread((user, departments, news, service, collections) => {
                    // 用户信息
                    ...
                    // 该用户权限下的所有服务数据
                    this.services.data = service.data.data;
        ...
        computed: {
            ...
            CurrPageServices: function() { 
                return this.servicesFilted.data.slice((this.servicesFilted.currPage - 1) * this.servicesFilted.pageSize, this.servicesFilted.currPage * this.servicesFilted.pageSize);
            },
            // 过滤器有无数据
            noResult: function() { 
        ...
        methods: {
            ...
            tagChangeEvent(id) { 
                ...
                let ele = Utils.domClass.hasClass(event.target, 'ivu-tag') ? event.target : event.target.parentElement;
                ...
                if (id != 'filter-by-order') {
                    ...
                    for (let groupId of tagGroupIds) {
                        ele = document.getElementById(groupId).getElementsByClassName('active')[0];
                        categoryKey = ele.getAttribute('category-key');
                        categoryType = ele.getAttribute('category-type');
                        if (categoryType != null && categoryType != '0') {
                            this.servicesFilted.data = [...this.filterBy(this.servicesFilted.data, categoryType, categoryKey)];
                    ...
                    this.servicesFilted.data = [...this.orderBy(this.servicesFilted.data, orderKey)];
                }
            },
            // 过滤器重置
            filterReset() { 
